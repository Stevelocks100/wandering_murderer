execute unless entity @n[tag=wander.sword_proj_direction,distance=0..0.1] on passengers facing entity @n[tag=wander.sword_proj_direction] feet facing ^ ^ ^-1 run rotate @s ~ ~
...
ride @n[tag=wander.sword_proj_display] mount @s
execute unless entity @s[nbt={inGround:1b}] run data modify entity @n[tag=wander.sword_proj_display] data.Motion set from entity @s Motion

execute if entity @s[nbt={inGround:1b}] on passengers run tag @s add wander.sword_proj_display_landed
execute if entity @s[nbt={inGround:1b}] unless entity @s[tag=wander.sword_proj_dont_explode] on passengers rotated as @s positioned ^ ^ ^0.5 run function wander:projectiles/sword/recursive_player_check
