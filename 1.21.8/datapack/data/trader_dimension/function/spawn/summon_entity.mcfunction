...
execute store result score random_mob trader_dimension.temp run random value 1..10
...
$execute if score random_mob trader_dimension.temp matches 7 run summon creeper 18 10 $(rand_z_pos) {attributes:[{id:"minecraft:fall_damage_multiplier",base:0.0d}],Motion:[$(rand_x_motion)d,$(rand_y_motion)d,0.0d],Tags:["trader_dimension.chest_mob"],DeathLootTable:"minecraft:empty"}
