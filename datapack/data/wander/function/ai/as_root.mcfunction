...
execute if score ai wander.data matches 0..28 if entity @s[nbt={OnGround:1b}] if score daytime wander.data matches -400..0 run function wander:ai/despawn
...
execute if score ai wander.data matches 20 if score attack_ai wander.data matches 1..2 unless entity @p[tag=wander.potential_target,distance=0..4] run scoreboard players set attack_ai wander.data 0