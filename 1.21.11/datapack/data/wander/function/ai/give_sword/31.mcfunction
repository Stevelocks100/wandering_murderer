...
execute unless entity @p[tag=wander.potential_target,distance=0..30] run scoreboard players set ai wander.data 34


execute unless entity @p[tag=wander.target] run tag @p[tag=wander.potential_target] add wander.target

execute as @a at @s if dimension wander:pocket run function wander:trader_dimension/leave